...
class GraphCLS_OGBGHIV_Walker(Walker):
    ...
    def criterion(self, y_hat: Tensor, y: Tensor) -> Tensor:
        # 将标签展平并转换为浮点型
        y = y.view(-1).float()
        # pos_weight = torch.tensor([25.0]).to(y.device)
        
        # 使用 BCEWithLogitsLoss
        return F.binary_cross_entropy_with_logits(y_hat, y) #, pos_weight=pos_weight)
    ...
    def evaluator(self, y_hat: Tensor, y: Tensor) -> Dict:
        ...
        preds = torch.softmax(y_hat, dim=-1)[:, 1]  # Select the probability for class 1
        # print("preds",preds)
        target = y.squeeze().float()  # Ensure target is of float type for ROC AUC
        # print("target",target)
        
        # Compute ROC AUC
        try:
            metric_val = roc_auc_score(target.cpu().numpy(), preds.cpu().numpy())
        except ValueError:  # In case all targets are the same, ROC AUC cannot be computed
...
class GraphCLS_OGBGHIV_Dataset(InMemoryDataset):
    def __init__(self, root, config, split: str = 'train' ,transform=None, pre_transform=None, meta_dict=None):
        ...
        '''
        self.name = 'ogbg-molhiv'  # original name, e.g., ogbg-molhiv
        ...
        super(GraphCLS_OGBGHIV_Dataset, self).__init__(self.root, transform, pre_transform)
