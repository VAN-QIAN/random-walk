...
class GraphCLS_OGBGPPA_Walker(Walker):
...
class GraphCLS_OGBGPPA_Dataset(InMemoryDataset):
    def __init__(self, root, config, split: str = 'train' ,transform=None, pre_transform=None, meta_dict=None):
        ...
        '''
        self.name = 'ogbg-ppa'  # original name, e.g., ogbg-molhiv
        ...
        super(GraphCLS_OGBGPPA_Dataset, self).__init__(self.root, transform, pre_transform)
    ...
    def process(self):
        ...
        for i, g in enumerate(data_list):
            g.x = torch.zeros(g.num_nodes, dtype=torch.long)
